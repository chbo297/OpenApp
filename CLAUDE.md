...
```bash
swift build
swift test        # 49 tests, OpenAPPCoreTests + OpenAPPUITests
```
...
AIAgent (facade)
  ...
  ├── ModelSelector?       "providerName/modelId" 格式，支持 primary + fallbacks
...
AISession (单次对话)
  ...
  └── sendMessage(text) → AsyncStream<AIAgentEvent>
        └── AIAgentExecutor (provider ↔ tool 循环)
...
AIAgent.init 接收两者作为参数（默认 `.default`），AISession 通过 `agentMask?.toolCentral` 访问。

## 核心类型速查

| 类型 | 文件 | 说明 |
|------|------|------|
| `ModelProvider` (protocol) | `Core/Model/ModelProvider.swift` | LLM provider 抽象，唯一实现: `AnthropicProvider` |
| `ModelConfiguration` | `Core/Model/ModelProvider.swift` | 模型配置 (id, name, reasoning, contextWindow, maxTokens) |
| `ModelProviderConfiguration` | `Core/Model/ModelProvider.swift` | Provider 配置 (baseURL, apiKey, apiProtocol, models) |
...
**自动注册 (14个):** clarify, memory, todo, file_read, file_write, file_search, skills_list, skill_view, skill_manage, text_to_speech, delegate_task, session_search, clipboard, haptic

**需宿主 app 注入 Provider (4个):** app_action, app_navigate, app_state, web_search, vision_analyze
...
| **改执行循环** | `Core/Agent/AIAgentExecutor.swift` runLoop() + `Core/Agent/ToolLoopDetector.swift` + `Core/Agent/ContextCompressor.swift` |
...
### Core/Agent/ — 核心编排 (14 files)

| 文件 | 职责 |
|------|------|
| `AIAgent.swift` | 顶层 facade：持有 session/memory/skills，组装 system prompt，注册内置工具 |
| `AIAgentExecutor.swift` | LLM ↔ tool 执行循环引擎：流式调用、工具执行、重试、回退、上下文压缩 |
...
| `AIAgentMask.swift` | session 创建时的不可变配置快照，解耦 session 与 Agent 运行时变更 |
...
### Core/Session/ — Session 生命周期 (4 files)

| 文件 | 职责 |
|------|------|
| `AISession.swift` | 单次对话：持有 messages、tools、uiState；sendMessage 创建 Executor 驱动对话 |
...
| 文件 | 职责 |
|------|------|
| `ModelProvider.swift` | ModelProvider 协议、ModelConfiguration、ProviderStreamEvent、ContentOrCacheControl、APIProtocol |
| `ModelProviderCentral.swift` | Provider 注册中心 actor：注册、解析 "providerName/modelId"、defaultPolicy |
...
### Core/Foundation/ — 共享基础设施 (8 files)
...
| 文件 | 职责 |
|------|------|
| `Tests/Core/OpenAPPCoreTests.swift` | 49+ 测试用例，覆盖 JSON、消息、Provider、SSE、存储、Agent、Memory、UIState、Prompt |
